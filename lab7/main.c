...
void backtracking( unsigned int index, int chosen[N], unsigned int permutation[N]  )    
{
	...
	if ( index == N ){        
		...
		// Your code goes here
		for(int i = 0; i < N; i++) // Loop through all possible index locations
		{
			if(chosen[i] == 0) // Check if current number has been selected
			{
				permutation[index] = i + 1;			// If current number has not been selected, set the permutation[index] to the number
				chosen[i] = 1;					// Set the number to 1 to indicate that it has now been selected
				backtracking(index+1, chosen, permutation);	// Recursive call at next index location
				chosen[i] = 0;					// Set chosen[i] back to 0 to start backtracking
			}
