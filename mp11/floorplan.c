 ...
 * INTRO PARAGRAPH
 * -------------------------------------------------------------------------------------------------------------------------------------
 * This program implements the game a slicing tree floorplan model.  10 functions and 1 helper function are used.  is_leaf_node() checks
 * if the current node is an external node.  is_internal_node() checks if the current node is an internal node.  is_in_subtree() checks
 * if tree is a subtree of another tree.  rotate() rotates the current module by 90 degrees.  recut() changes the current cutline to the
 * opposite cutline.  swap_module() swaps the modules of two nodes.  swap_topography() swaps two subtrees.  get_expression() gets the 
 * postfix expression from the postfix_traversal() function.  init_slicing_tree() initializes the slicing tree.  get_total_resource()
 * evaluates the total resources of all the modules.  The helper function areSame() checks if two subtrees are identical
 */
...
int is_leaf_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
	...
	if(ptr->cutline == UNDEFINED_CUTLINE) // Check if node is a cutline or a module
		return 1; // If the node is a module, then it is a leaf node
	return 0;
}
...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
	...
	if(ptr->module == NULL) // Check if node is a cutline or a module
		return 1; // If node is a cutline, then it is an internal node
	return 0;
}

int areSame(node_t * a, node_t * b) // Helper function to check if two subtrees are the same
{ 
    // base cases
    if (a == NULL && b == NULL) // If both nodes are null, then subtrees are the same
        return 1; 
  
    if (a == NULL || b == NULL) // If only one of the nodes is null, but not the other, then subtrees are not the same
        return 0; 
  
    // Check if the modules and cutlines of both nodes are the same and modules and cutlines of left and right subtrees are also the same
    return (a->module == b->module && a->cutline == b->cutline && areSame(a->left, b->left) && areSame(a->right, b->right)); 
}
...
int is_in_subtree(node_t* a, node_t* b) {
  // TODO: (remember to modify the return value appropriately)
	...
	if(a == NULL) // Check if a is null
		return 0; // If a is null, then the subtree at root b can't be a subtree of a
	
	if(b == NULL) // Check if b is null
		return 1; // If b is null, then it is a subtree of a
	
	if(areSame(a, b) == 1) // Call areSame function to check if subtrees a and b are the same
		return 1; // If the subtrees are the same, then b is a subtree of a

	int correct = is_in_subtree(a->left, b) || is_in_subtree(a->right, b); // Recursive call of left and right subtree of a
...
void rotate(node_t* ptr) {
  // TODO: 
	...
	int temp = ptr->module->w; // Make temp int to hold original width
	ptr->module->w = ptr->module->h; // Set width to the height
	ptr->module->h = temp; // Set height to original height
}
...
void recut(node_t* ptr) {
  ...
  // TODO:
	...
	if(ptr->cutline == H) // Check if node->cutline is a horizontal cut or a vertical cut
		ptr->cutline = V; // If it is a horizontal cut, change it to a vertical cut
	else
		ptr->cutline = H; // If it is a vertical cut, change it to a horizontal cut
...
void swap_module(node_t* a, node_t* b) {
  ...
  assert(b->module != NULL && b->cutline == UNDEFINED_CUTLINE);
	...
	module_t * temp; // Make temporary module
	temp = a->module; // Set temporary module to the module stored at a
	a->module = b->module; // Set the module stored at a to the module stored at b
	b->module = temp; // Set the module stored at b to the original module stored at a
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // TODO:
	...
	int left1 = 0, right1 = 0; // Declare variables to check whether a is a left child or a right child and inititialize to 0
	int left2 = 0, right2 = 0; // Declare variables to check whether b is a left child or a right child and inititialize to 0

	if(a == a->parent->left) // Check if a is a left child or a right child
		left1 = 1;
	else
		right1 = 1;

	if(b == b->parent->left) // Check if b is a left child or a right child
		left2 = 1;
	else
		right2 = 1;

	node_t * temp; // Make temporary node
	temp = a->parent; // Set temporary node to the parent of a
	a->parent = b->parent; // Set the parent of a to the parent of b
	b->parent = temp; // Set the parent of b to the original parent of a
	
	if(left2 == 1) // If b was a left child, then make a the left child of the original parent of b
		a->parent->left = a;
	else if(right2 == 1) // If b was a right child, then make a the right child of the original parent of b 
		a->parent->right = a;		

	if(left1 == 1) // If a was a left child, then make b the left child of the original parent of a
		b->parent->left = b;
	else if(right1 == 1) // If a was a right child, then make b the right child of the original parent of a
		b->parent->right = b;
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  // TODO:
	...
	postfix_traversal(ptr->left, nth, expression); // Recursive call over the left subtree
	postfix_traversal(ptr->right, nth, expression); // Recursive call over the right subtree

	if(ptr->cutline == UNDEFINED_CUTLINE) // Check if current node is a cutline or a module
		expression[*nth].module = ptr->module; // If current node is a module, then set the current position in the expression array to the module
	else
		expression[*nth].cutline = ptr->cutline; // If current node is a cutline, then set the current position in the expression array to the cutline

	(*nth)++; // Increment index pointer
...
{
  // TODO:
	int total_resource = 0; // Declare variable to hold total_resource and initialize to 0
	
	if(ptr == NULL) // Check if current node is null
		return 0; // If current node is null, return 0

	total_resource += get_total_resource(ptr->left); // Set total_resource to total_resource + the total resource of the left subtree
	total_resource += get_total_resource(ptr->right); // Set total_resource to total_resource + the total resource of the right subtree

	if(ptr->cutline == UNDEFINED_CUTLINE) // If the current node is a module
		total_resource += ptr->module->resource; // Add the resource stored in the module to total_resource

	return total_resource; // Return the total_resource
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  // TODO:
	...
	node_t * newNode = (node_t *)malloc(sizeof(node_t)); // Allocate memory to newNode

	if(n == num_modules - 1) // Check if n is the last node
	{
		newNode->module = &(modules[n]); // If n is the last node, set the module stored at newNode to the address of the module stored in the modules array
		newNode->cutline = UNDEFINED_CUTLINE; // If n is the last node, set the cutline stored at newNode to UNDEFINED_CUTLINE
		return newNode; // Return newNode
	}

	newNode->module = NULL; // If newNode is an internal node, set the module stored at newNode to NULL

	newNode->right = (node_t *)malloc(sizeof(node_t)); // Allocate memory to the right child of the node
	newNode->right->module = &(modules[n]); // Set the module stored at the right child to the address of the module stored in the modules array
	newNode->right->cutline = UNDEFINED_CUTLINE; // Set the cutline stored at the right child to UNDEFINED_CUTLINE

	newNode->left = init_slicing_tree(newNode, n+1); // Recursive call to the left child of the node
